...
export function saveToJSON() {
  try {
    ...
    // ── Collect duties and tasks from appState (works in both Table & Card View) ──
  ...
  try {
    ...
    reader.onload = function (e) {
      try {
        ...
        // ── Duties & Tasks — state-first (works in both Table & Card View) ──
        appState.dutiesData = [];
        ...
        if (data.duties && data.duties.length > 0) {
          data.duties.forEach(dutyData => {
            ...
            appState.dutiesData.push({
              ...
              title: String(dutyData.duty || '').trim(),
              tasks
            });
          });
        } else {
          // Empty file — add one blank duty+task as default
          appState.dutyCount = 1;
          const dutyId = 'duty_1';
          appState.taskCounts[dutyId] = 1;
          appState.dutiesData = [{
            id: dutyId, num: 1, title: '',
        ...
        // Sync DOM values → appState (belt-and-suspenders)
        syncAllFromDOM();
