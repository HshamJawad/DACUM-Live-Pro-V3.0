...
export function restoreSnapshot(id) {
  ...
  renderAll();
  resetHistoryToCurrentState();   // new baseline = restored state
  renderSnapshotPanel();
...
export function renderSnapshotPanel() {
  ...
  if (snaps.length === 0) {
    list.innerHTML = `<p style="color:#9ca3af;font-size:0.9em;text-align:center;padding:16px 0;">
      No snapshots yet. Click <strong>+ Save Snapshot</strong> to create one.
    </p>`;
    return;
  }
  ...
  list.innerHTML = snaps.map(s => {
    ...
    return `
      <div class="snap-item" data-snap-id="${s.id}">
        <div class="snap-meta">
          <span class="snap-name">${_esc(s.name)}</span>
          <span class="snap-date">${dateStr} · ${timeStr}</span>
        </div>
        <div class="snap-actions">
          <button class="snap-btn snap-restore" data-action="restore-snapshot" data-snap-id="${s.id}"
                  title="Restore this checkpoint">↩ Restore</button>
          <button class="snap-btn snap-delete"  data-action="delete-snapshot"  data-snap-id="${s.id}"
                  title="Delete this checkpoint">🗑</button>
        </div>
      </div>`;
  }).join('');
}
