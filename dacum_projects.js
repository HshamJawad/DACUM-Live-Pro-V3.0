...
function _applyLiveWorkshopDOM(s) {
  ...
  if (sessionId && isFinalized) {
    // ── Project has an active session — restore the live section ──
    ...
    if (lwLinkEl && participantUrl) {
      // Derive short link from full URL
    ...
    // Show results if we have aggregated data
    ...
    if (btnReset)    btnReset.style.display = s.verificationDecisionMade ? 'inline-block' : 'none';

  } else {
    // ── No session for this project — reset live section ──
    ...
    // Reset finalize button so user can create a new session
