...
function _startInlineRename(id) {
  ...
  // Guard: mousedown inside input must NOT trigger blur-commit
  let _mouseInsideInput = false;
  nameInput.addEventListener('mousedown', function () { _mouseInsideInput = true; });
  nameInput.addEventListener('mouseup',   function () { _mouseInsideInput = false; });

  function commit() {
    if (_mouseInsideInput) return;   // user clicked inside — keep editing
  ...
  function cleanup() {
    ...
    nameInput.removeEventListener('mousedown', function(){});
    nameInput.removeEventListener('mouseup',   function(){});
  }

  function onKey(e) {
    if (e.key === 'Enter')  { e.preventDefault(); _mouseInsideInput = false; commit(); }
...
/* Hide icon when user has typed something */
.dps-search-box.has-value .dps-search-icon { opacity: 0; }
.dps-search {
  width: 100%;
...
/* When typing, remove icon padding so text starts at left */
.dps-search-box.has-value .dps-search { padding-left: 10px; }
