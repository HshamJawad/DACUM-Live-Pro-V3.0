...
function _toggleSidebar() {
  ...
  sb.classList.toggle('dps-expanded', !collapsed);
  if (btn) btn.textContent = collapsed ? '▶' : '◀';
}

function _positionToggle() {
  // Nothing needed — button is position:absolute inside the sidebar
}
...
/* Main app container must not overflow */
...
/* Tabs must wrap, never overflow the container */
.tabs {
...
.dps-sidebar {
  position: sticky;
  top: 56px;                     /* sits below the fixed toolbar */
  height: calc(100vh - 56px);   /* full remaining viewport height */
  overflow-y: auto;
  overflow-x: visible;
  background: #1e1e2e;
  color: #cdd6f4;
...
.dps-expanded  { width: 260px; }
.dps-collapsed { width: 48px; }
...
/* ── Search ── */
.dps-search-wrap { padding: 8px 10px 4px; }
.dps-search {
  width: 100%;
  padding: 6px 10px;
  border-radius: 6px;
...
/* ── Toggle button — anchored to right edge of sidebar ── */
.dps-toggle {
  position: absolute;
  top: 50%;
  right: -16px;                  /* sits on the edge, half outside */
...
.dps-toggle:hover { background: #cba6f7; color: #1e1e2e; }
...
/* ── Toggle button — fixed to the RIGHT EDGE of sidebar, vertically centred ── */
.dps-toggle {
  position: fixed;
  top: 50%;
  transform: translateY(-50%);
...
.dps-toggle:hover { background: #cba6f7; color: #1e1e2e; }
.dps-expanded  .dps-toggle,
.dps-collapsed .dps-toggle { left: unset; }
`;
