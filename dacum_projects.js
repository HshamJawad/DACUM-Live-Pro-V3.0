...
function _applyLiveWorkshopDOM(s) {
  ...
  if (sessionId && isFinalized) {
    ...
    // Finalize button: disabled (session already running), others enabled
    if (btnFinalize) { btnFinalize.disabled = true;  btnFinalize.title = 'Session already active for this project'; }
    if (btnBypass)   { btnBypass.disabled   = false; btnBypass.title   = ''; }
    ...
    // All action buttons fully enabled
    if (btnFinalize) { btnFinalize.disabled = false; btnFinalize.title = ''; }
    if (btnBypass)   { btnBypass.disabled   = false; btnBypass.title   = ''; }
    if (btnReset)    btnReset.style.display = hasDecision ? 'inline-block' : 'none';
  }
}
