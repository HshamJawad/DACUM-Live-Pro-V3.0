...
export function importProjectFromData(data, fileName) {
  const label = (data?.chartInfo?.occupationTitle || '').trim()
  ...
  const s = data;
  const state = {
    dutiesData:               s.duties ? _dutiesArrayToState(s.duties) : (s.dutiesData || []),
    dutyCount:                s.dutiesData ? s.dutiesData.length : (s.duties ? s.duties.length : 0),
  ...
  // Recalculate taskCounts from duties
