...
export function loadDutiesForVerification() {
  const container  = document.getElementById('verificationAccordionContainer');
  const dutiesData = appState.dutiesData || [];

  // Filter duties that have a title and at least one non-empty task
  const populated = dutiesData.filter(d =>
    d.title.trim() && d.tasks.some(t => t.text.trim())
  );
  ...
  for (const duty of populated) {
    const dutyId   = duty.id;
    const dutyText = duty.title.trim();
    totalDuties++;
...
export function refreshDashboard() {
  ...
  if (summaryEl && validResults.length > 0) {
    ...
    summaryEl.innerHTML = `
      ...
      <div class="summary-card"><h4>Formula Used</h4><p>${appState.priorityFormula === 'if' ? 'I × F' : 'I × F × D'}</p></div>`;
  }
