...
export function renderDashboardProjectSelector() {
  ...
  // Read saved projects from localStorage (same key dacum_projects uses)
  let projects = [];
  ...
  // Only show projects that have voting results
  const withResults = projects.filter(p =>
    p.state?.workshopResults && Object.keys(p.state.workshopResults).length > 0
  );

  sel.innerHTML = '';

  // Always add "Current project" option at top
  const currentOpt = document.createElement('option');
  currentOpt.value = '__current__';
  currentOpt.textContent = '📊 Current project';
  ...
  withResults.forEach(p => {
    ...
    if (p.id === activeId) opt.textContent += ' ✓';
    sel.appendChild(opt);
  });

  // Pre-select active project if it has results
  const hasCurrentResults = Object.keys(appState.workshopResults || {}).length > 0;
  if (!hasCurrentResults && withResults.length > 0) {
    sel.value = withResults[0].id;
  } else {
    sel.value = '__current__';
  }

  // Show/hide selector based on whether any saved projects have results
  const wrap = document.getElementById('dashboardProjectSelectorWrap');
  if (wrap) wrap.style.display = withResults.length > 0 ? 'flex' : 'none';
}
...
export function refreshDashboard() {
  ...
  const sel = document.getElementById('dashboardProjectSelector');
  const selectedId = sel?.value || '__current__';
...
document.addEventListener('dacum:project-loaded', function (e) {
  // Give _applyState a tick to finish setting appState before we read it
  setTimeout(function () {
    // Rebuild selector options
    renderDashboardProjectSelector();

    // Auto-select the newly loaded project in the dropdown (if it has results)
    const sel       = document.getElementById('dashboardProjectSelector');
    const projectId = e.detail?.projectId;
    if (sel && projectId) {
      // Check if this project is in the selector options
    ...
    // Refresh the dashboard with the correct source
    refreshDashboard();
  }, 50);
});
