...
import { pushCommand, isActive } from './history.js';

// ── Snapshot helpers (serialise DOM + state for structural undo/redo) ─────────

function _snapshot() {
  const container = document.getElementById('dutiesContainer');
  if (!container) return { html: '', dutyCount: 0, taskCounts: {} };

  // Persist live .value into HTML attribute so innerHTML captures it
...
function _restore(snap) {
  ...
  appState.taskCounts = { ...snap.taskCounts };
}
...
export function addDuty() {
  ...
  if (!isActive()) {
    ...
    pushCommand({ label: `Add Duty ${appState.dutyCount}`, undo: () => _restore(before), redo: () => _restore(after) });
  }
}
...
export function removeDuty(dutyId) {
  ...
  if (!isActive()) {
    const after = _snapshot();
    pushCommand({ label: `Remove Duty`, undo: () => _restore(before), redo: () => _restore(after) });
  }
}
...
export function addTask(dutyId) {
  ...
  if (!isActive()) {
    const after = _snapshot();
    pushCommand({ label: `Add Task ${taskNum}`, undo: () => _restore(before), redo: () => _restore(after) });
  }
}
...
export function removeTask(taskDivId) {
  ...
  if (!isActive()) {
    const after = _snapshot();
    pushCommand({ label: `Remove Task`, undo: () => _restore(before), redo: () => _restore(after) });
  }
}
...
export function clearDuty(dutyId) {
  if (confirm('Are you sure you want to clear this duty and all its tasks?')) {
    const before = _snapshot();
    const dutyInput = document.querySelector(`input[data-duty-id="${dutyId}"]`);
    if (dutyInput) dutyInput.value = '';
    document.querySelectorAll(`[data-task-id^="${dutyId}_"]`).forEach(ti => { ti.value = ''; });
    const after = _snapshot();
    pushCommand({ label: 'Clear Duty', undo: () => _restore(before), redo: () => _restore(after) });
