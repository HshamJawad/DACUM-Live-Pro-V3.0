...
function _renderCardView(container) {
  ...
  (appState.dutiesData || []).forEach(duty => {
    ...
    tasksArea.appendChild(addTaskBtn);
...
function _setAddDutyVisibility(mode) {
  // Original table-mode Add Duty button
  const orig = document.getElementById('btnAddDuty');
  // Card-mode Add Duty button (injected once)
  ...
  if (mode === 'card') {
    ...
    // Wire click — idempotent (remove first to avoid duplicates)
    cardBtn.onclick = null;
    cardBtn.addEventListener('click', () => {
      document.getElementById('btnAddDuty')?.click();
    });
  } else {
