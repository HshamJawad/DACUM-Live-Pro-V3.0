...
import { saveCurrentProject,
         renderProjectsSidebar }                           from './dacum_projects.js';
...
export function setupEvents() {
  ...
  _on('aiGenerateBtn', 'click', () => {
    generateAIDacum()
      .then(() => { saveCurrentProject(); renderProjectsSidebar(); })
      .catch(() => {});
  });
  ...
  _on('loadFileInput', 'change', (e) => {
    loadFromJSON(e);
    setTimeout(() => { saveCurrentProject(); renderProjectsSidebar(); }, 600);
  });
  ...
  if (dutiesCont) {
    ...
    dutiesCont.addEventListener('input', function (e) {
      const t = e.target;
      if      (t.matches('input[data-duty-id]')) syncDutyTitle(t.getAttribute('data-duty-id'), t.value);
      else if (t.matches('input[data-task-id]')) syncTaskText(t.getAttribute('data-task-id'), t.value);
      else return;
