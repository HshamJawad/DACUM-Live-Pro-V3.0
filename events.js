...
import { undo, redo, pushCommand, makeTextCommand, clearHistory } from './history.js';
...
export function setupEvents() {
  ...
  if (dutiesCont) {
    ...
    // ── Word-level text undo tracking (Word-like behaviour) ──────────────
    const _tt = { el: null, before: '', timer: null };
    const TEXT_DELAY = 1500; // ms of silence → commit word

    function _ttCommit() {
      clearTimeout(_tt.timer);
      _tt.timer = null;
      if (!_tt.el || !document.contains(_tt.el)) { _tt.el = null; return; }
      const cur = _tt.el.value;
      if (cur !== _tt.before) {
        pushCommand(makeTextCommand(_tt.el, _tt.before, cur));
        _tt.before = cur;
      }
    }
    ...
    dutiesCont.addEventListener('focusin', function (e) {
      ...
      if (_tt.el && _tt.el !== t) _ttCommit();   // commit prev input
      _tt.el     = t;
      _tt.before = t.value;
    });

    dutiesCont.addEventListener('focusout', function (e) {
      if (e.target === _tt.el) { _ttCommit(); _tt.el = null; }
    });

    dutiesCont.addEventListener('keydown', function (e) {
      if (e.target !== _tt.el) return;
      if (e.key === ' ' || e.key === 'Enter') {
        // Commit after the character lands in the input
        clearTimeout(_tt.timer);
        _tt.timer = setTimeout(_ttCommit, 0);
      } else {
        clearTimeout(_tt.timer);
        _tt.timer = setTimeout(_ttCommit, TEXT_DELAY);
      }
    });
    // ────────────────────────────────────────────────────────────────────
  }
