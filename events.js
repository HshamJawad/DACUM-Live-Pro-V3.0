...
import { addDuty, addTask, removeDuty, removeTask, clearDuty,
         syncAllFromDOM, syncDutyTitle, syncTaskText } from './duties.js';
...
export function setupEvents() {
  ...
  // Keyboard shortcuts (only active when duties tab is visible)
  ...
  if (dutiesCont) {
    ...
    // Structural clicks — sync DOM → state, push history, then mutate
    dutiesCont.addEventListener('click', function (e) {
      ...
      if (target.matches('[data-action="add-task"]')) {
        ...
        pushHistoryState();                              // syncAllFromDOM is inside removeDuty
        removeDuty(target.getAttribute('data-duty-id'));

      } else if (target.matches('[data-action="remove-task"]')) {
        pushHistoryState();                              // syncAllFromDOM is inside removeTask
        removeTask(target.getAttribute('data-task-div-id'));

      } else if (target.matches('[data-action="clear-duty"]')) {
        pushHistoryState();                              // syncAllFromDOM is inside clearDuty
        clearDuty(target.getAttribute('data-duty-id'));
      }
    });

    // ── Word-level text undo (burst model) ───────────────────
    // First keystroke after 120 ms silence → new undo checkpoint.
    ...
    dutiesCont.addEventListener('input', function (e) {
      const t = e.target;
      if (t.matches('input[data-duty-id]')) {
        syncDutyTitle(t.getAttribute('data-duty-id'), t.value);
      } else if (t.matches('input[data-task-id]')) {
