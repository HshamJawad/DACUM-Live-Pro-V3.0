...
import { pauseRecording, resumeRecording, clearHistory } from './history.js';
...
Generate the DACUM draft now in valid JSON format only.`;
  ...
  try {
    ...
    // Clear existing duties and populate with AI data
    document.getElementById('dutiesContainer').innerHTML = '';
    appState.dutyCount  = 0;
    appState.taskCounts = {};
    ...
    dacumData.duties.forEach(dutyData => {
      ...
      if (dutyInput && dutyData.title) dutyInput.value = dutyData.title;
